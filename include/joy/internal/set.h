 ...
 * weak ordering.
 */
#define JOY_SET(value_comp) (value_comp, JOY_TREE())

/*!
 * Return the macro used by a set to sort its element in strict weak ordering.
 */
#define JOY_SET_VALUE_COMP(set) CHAOS_PP_TUPLE_ELEM(2, 0, set)

#define JOY_I_SET_TREE(set) CHAOS_PP_TUPLE_ELEM(2, 1, set)
#define JOY_I_SET_SET_TREE(set, tree) CHAOS_PP_TUPLE_REPLACE(1, set, tree)
...
#define JOY_SET_SIZE_S(s, set) \
    JOY_TREE_SIZE_S(JOY_I_SET_TREE(set))
